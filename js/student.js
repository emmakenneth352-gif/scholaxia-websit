...
(function () {
  ...
  function renderLiveCard(c, isLive) {
    ...
    return (
      ...
      "</span>" +
      (vis ? '<span style="opacity:.9;font-size:.75rem;font-weight:700">' + esc(vis) + "</span>" : "") +
      "</div>" +
      ...
      '<div class="card-foot">' +
      (isLive
        ? '<button type="button" class="btn btn-primary btn-mini" data-join-live="' + esc(c.id) + '">Join now</button>'
  ...
  function showLiveJoinResult(res) {
    ...
    var joinUrl = res.join_url || res.meeting_link || res.url || "";
    ...
    panel.innerHTML =
      ...
      (res.host_name ? "<span>Host: " + esc(res.host_name) + "</span>" : "") +
      "</div>" +
      '<div class="btn-row">' +
      (joinUrl
        ? '<a class="btn btn-primary" href="' + esc(joinUrl) + '" target="_blank" rel="noopener">Open classroom</a>'
        : '<span class="muted">Waiting for the host to start the stream…</span>') +
    ...
    if (saveBtn) {
      saveBtn.addEventListener("click", function () {
        ...
        saved.unshift({
          id: res.id || res.session_id || Date.now(),
          title: title,
          url: joinUrl,
