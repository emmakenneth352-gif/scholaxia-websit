...
(function () {
  ...
  async function hostClass(goLiveNow) {
    ...
    try {
      ...
      if (goLiveNow && created && created.id) {
        var code = created.join_code ? " Access code: " + created.join_code + "." : "";
        if (confirm("Class is live!" + code + " Open classroom?")) {
  ...
  async function loadStudents() {
    ...
    try {
      ...
      if (!Array.isArray(rows) || !rows.length) {
        el.innerHTML =
          '<div class="empty">No students yet. When admin assigns a student, they appear here.</div>';
        return;
      }
      el.innerHTML = rows
        .map(function (r) {
