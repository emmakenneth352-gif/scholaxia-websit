...
import '../../utils/web_embed_player.dart';

class VideoTutorialsScreen extends StatefulWidget {
  const VideoTutorialsScreen({super.key, this.kindAudience = false});

  final bool kindAudience;
...
class _VideoTutorialsScreenState extends State<VideoTutorialsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
        final rows = widget.kindAudience
          ? await _api.kindVideoTutorials()
          : await _api.videoTutorials();
      if (!mounted) return;
      setState(() {
        _videos = rows;
        _loading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          widget.kindAudience ? 'Kids Video Lessons' : 'Video Tutorials',
...
class _VideoPlayerScreenState extends State<_VideoPlayerScreen> {
  ...
  /// webview_flutter has no implementation for Windows desktop.
  ...
  void initState() {
    ...
    if (widget.isDirectVideo) {
      ...
      // Web renders the video in an iframe (WebEmbedPlayer) — no controller.
    } else if (_isWindowsDesktop) {
      // webview_flutter is unsupported on Windows — offer browser playback.
  ...
  Widget _buildWebPlayer() {
    if (kIsWeb) {
      return WebEmbedPlayer(url: widget.embedUrl);
    }
    if (_externalPlayback) {
      return _buildExternalPlayback();
    }
  ...
  Widget _buildExternalPlayback() {
    return Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.open_in_new, color: Colors.white70, size: 36),
          const SizedBox(height: 12),
          const Text(
