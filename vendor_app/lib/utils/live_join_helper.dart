...
/// Join a live class directly by id — used in the Kids app where children
/// never type codes. The kid's account (created with the parent's email) is
/// the credential; the class was booked for them by the tutor.
Future<void> joinLiveDirectly(
  BuildContext context,
  ApiService api,
  Map<String, dynamic> liveClass,
) async {
  final classId = liveClass['id']?.toString() ?? '';
  if (classId.isEmpty || !context.mounted) return;

  try {
    LiveClassRingService.instance.stop();
    final data = await api.joinLiveClassById(classId);
    final userId = await api.getUserId() ?? 'kid';
    ...
    await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => LiveClassScreen(
          ...
          livekitToken:
              data['livekit_token']?.toString() ?? data['token']?.toString(),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          e is ApiException ? e.message : 'Could not join this class yet.',
        ),
      ),
    );
  }
